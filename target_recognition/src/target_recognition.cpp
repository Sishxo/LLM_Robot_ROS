...
namespace Target_Recognition
{
    ...
    int ROS_Target_Recognition::Start(void)
    {
        ...
        while (ros::ok())
        {
            ...
            ros::Time imageTime;
            ros::Time targetTime;
            if (getImageData(rgbImage, depthImage) == 0)
                continue;
            else
            {
                ROS_INFO("获取到图像信息");
    ...
    int ROS_Target_Recognition::rosTargetRecPublish(ros::Time controlTime, ros::Time imageTime, ros::Time targetTime, ImageTarget data)
    {
        ...
        if (target_msg.recognition_status == 0)
            ROS_INFO("目标检测失败");
    ...
    XarmErrorState ROS_Target_Recognition::errorDetection(void)
    {
        ...
        if ((ImageTime_ - ImageRealTime_) > ros::Duration(ImageDevice_))
            return XarmErrorState_ERROR;
        else
            return XarmErrorState_NORMAL;
    }
