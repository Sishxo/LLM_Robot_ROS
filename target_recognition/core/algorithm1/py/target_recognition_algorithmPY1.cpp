...
namespace Target_Recognition
{
    ...
    /// @param path 
    void Image_AlgorithmPY1::LoadPython(std::string path, std::string module_name, std::string class_name)
    {
    ...
    /// @brief 获取当前硬件平台的名称
    /// @return 返回当前硬件平台的名称
    std::string Image_AlgorithmPY1::Name()
    {
        std::string name ;
    ...
    /// @brief 获取当前硬件平台的名称
    /// @return 返回当前硬件平台的名称
    ImageTarget Image_AlgorithmPY1::Do(cv::Mat &rgbImage, cv::Mat &depthImage, int objectID)
    {
    ...
    /// @brief 获取当前硬件平台的名称
    /// @return 返回当前硬件平台的名称
    int Image_AlgorithmPY1::ErrorDetect()
    {
        pybind11::object result = Algorithm_Init.attr("ErrorDetect")();
        return result.cast<int>();
    }

    pybind11::array Image_AlgorithmPY1::mat_to_array(cv::Mat &mat) {
    ...
    ImageTarget Image_AlgorithmPY1::getData()
    {
